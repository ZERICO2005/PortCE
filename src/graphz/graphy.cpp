...
void GraphY::gfz_ShiftRight(uint32_t pixels) {
    if (pixels == 0) { return; }
    int32_t shift = static_cast<int32_t>(pixels);
    ...
    size_t copy_size = static_cast<size_t>(y1 - y0);
    size_t line_count = static_cast<size_t>(x1 - x0);

    uint8_t * buf = get_vBuffer() + y0 + ((x1 - 1) * GFZ_LCD_HEIGHT);
...
void GraphY::gfz_ShiftLeft(uint32_t pixels) {
    ...
    uint8_t * buf = get_vBuffer() + y0 + (x0 * GFZ_LCD_HEIGHT);
    uint8_t const * src = buf + (shift * GFZ_LCD_HEIGHT);
    uint8_t       * dst = buf;
    for (size_t i = 0; i < line_count; i++) {
        memcpy(dst, src, copy_size);
...
void GraphY::gfz_SetPixel_NoClip(uint32_t x, uint8_t y, uint8_t color) {
    if (x < GFZ_LCD_WIDTH && y < GFZ_LCD_HEIGHT) {
        (get_vBuffer())[x * GFZ_LCD_HEIGHT + y] = color;
    }
}
...
void GraphY::gfz_SetPixel(uint32_t x, uint8_t y) {
    if (x < GFZ_LCD_WIDTH && y < GFZ_LCD_HEIGHT) {
        (get_vBuffer())[x * GFZ_LCD_HEIGHT + y] = lib.Color;
    }
}

template<>
uint8_t GraphY::gfz_GetPixel(uint32_t x, uint8_t y) {
    return (get_vBuffer())[(x & 0xFFFF) * GFZ_LCD_HEIGHT + y];
}
...
void GraphY::gfz_HorizLine_NoClip(uint32_t x, uint8_t y, uint32_t length) {
    uint8_t *fill = get_vBuffer() + (x * GFZ_LCD_HEIGHT) + static_cast<uint32_t>(y);
...
void GraphY::gfz_VertLine_NoClip(uint32_t x, uint8_t y, uint32_t length) {
    uint8_t *fill = get_vBuffer() + (static_cast<uint32_t>(y) + (x * GFZ_LCD_HEIGHT));
...
static void gfz_internal_PrintChar_NoClip(const char c, const uint8_t charWidth) {
    const uint8_t *bitImage = lib.TextData + lib.Maximum_Font_Height * static_cast<unsigned char>(c);
    uint8_t *fillLinePtr = get_vBuffer() + (lib.TextYPos + (lib.TextXPos * GFZ_LCD_HEIGHT));
...
void GraphY::gfz_FloodFill(uint32_t x, uint8_t y, const uint8_t color) {
    ...
    if (
        static_cast<int32_t>(x) < lib.ClipXMin || static_cast<int32_t>(x) >= lib.ClipXMax ||
        static_cast<int32_t>(y) < lib.ClipYMin || static_cast<int32_t>(y) >= lib.ClipYMax
    ) {
        return;
    }
    const uint8_t ff_ClipYMin = static_cast<uint8_t>(lib.ClipYMin);
    const uint8_t ff_ClipYMax = static_cast<uint8_t>(lib.ClipYMax);
    uint8_t * const buffer = get_vBuffer();
    ...
    FloodFillCord* const ff_stack = reinterpret_cast<FloodFillCord*>(alloca(FLOODFILL_STACK_SIZE));
    size_t stack_size = 1;
    ff_stack[0] = (FloodFillCord){static_cast<uint32_t>(x), y};
    int32_t x1;
    bool spanAbove, spanBelow;
    const uint8_t ff_ClipYMaxMinus1 = static_cast<uint8_t>(ff_ClipYMax - 1);
    do {
        stack_size--;
        const FloodFillCord ff_cord = ff_stack[stack_size];

        x1 = static_cast<int32_t>(ff_cord.x);
            ...
            if (ff_cord.y > ff_ClipYMin) {
                ...
                if (!spanAbove && src_pixel == replace) {
                    ff_stack[stack_size] = (FloodFillCord){
                        ...
                        static_cast<uint8_t>(ff_cord.y - 1)
                    };
                    stack_size++;
                    spanAbove = true;
...
gfz_sprite_t *GraphY::gfz_GetSprite(gfz_sprite_t *sprite_buffer, int32_t x, int32_t y) {
    ...
    const uint8_t* src_buf = get_vBuffer() + y + (x * GFZ_LCD_HEIGHT);
...
gfz_sprite_t *GraphY::gfz_RotateSpriteC(const gfz_sprite_t *sprite_in, gfz_sprite_t *sprite_out) {
    ...
    const uint8_t* src_ptr = reinterpret_cast<const uint8_t*>(&sprite_in->data);
    for (uint8_t y = 0; y < sprite_in->height; y++) {
        for (uint8_t x = 0; x < sprite_in->width; x++) {
...
void GraphY::gfz_Sprite_NoClip(const gfz_sprite_t *sprite, uint32_t x, uint8_t y) {
    ...
    uint8_t* dst_buf = get_vBuffer() + y + (x * GFZ_LCD_HEIGHT);
...
void GraphY::gfz_Sprite(const gfz_sprite_t * sprite, int32_t x, int32_t y) {
    ...
    if (
        ...
        gfz_Sprite_NoClip(sprite, static_cast<uint32_t>(x), static_cast<uint8_t>(y));
        return;
    }
    gfz_Wait();
    ...
    uint8_t* dst_buf = get_vBuffer() + (y + min_clipY) + ((x + min_clipX) * GFZ_LCD_HEIGHT);
...
void GraphY::gfz_TransparentSprite(const gfz_sprite_t * sprite, int32_t x, int32_t y) {
    ...
    if (
        ...
        gfz_TransparentSprite_NoClip(sprite, static_cast<uint32_t>(x), static_cast<uint8_t>(y));
        return;
    }
    gfz_Wait();
    ...
    const uint8_t* src_buf = sprite->data + min_clipY + (min_clipX * sprite->height);
    const uint8_t clip_jumpY = static_cast<uint8_t>(min_clipX + max_clipX);
...
void GraphY::gfz_ScaledSprite_NoClip(
    ...
    gfz_Wait();
    const uint8_t* src_buf = sprite->data;
    uint8_t* buf_start = get_vBuffer() + y + (x * GFZ_LCD_HEIGHT);
...
gfz_sprite_t *GraphY::gfz_RotateScaleSprite(
    ...
    const float sin_f = static_cast<float>(sinf(static_cast<float>(angle) * 0.0245436926f));
    const float cos_f = static_cast<float>(cosf(static_cast<float>(angle) * 0.0245436926f));

    float out_center = static_cast<float>(out_size) / 2.0f;
    uint8_t* dst_ptr = sprite_out->data;
    for (uint8_t y = 0; y < out_size; y++) {
        float yc = (static_cast<float>(y) - out_center) / static_cast<float>(out_size);
        for (uint8_t x = 0; x < out_size; x++) {
            float xc = (static_cast<float>(x) - out_center) / static_cast<float>(out_size);
            float x_pos = (xc * cos_f - yc * sin_f) + 0.5f;
            float y_pos = (yc * cos_f + xc * sin_f) + 0.5f;
            x_pos *= static_cast<float>(in_size);
            y_pos *= static_cast<float>(in_size);
            int32_t x_write = static_cast<int32_t>(x_pos);
            int32_t y_write = static_cast<int32_t>(y_pos);
            if (
...
uint8_t GraphY::gfz_RotatedScaledSprite_NoClip(const gfz_sprite_t *sprite, uint32_t x0, uint8_t y0, uint8_t angle, uint8_t scale) {
    ...
    const float angle_f = static_cast<float>(angle) * 0.0245436926f;

    const float cos_f = static_cast<float>(cosf(angle_f));
    const float sin_f = static_cast<float>(sinf(angle_f));

    const float in_size_f = static_cast<float>(in_size);
    const float out_size_f = static_cast<float>(out_size);
    ...
    for (uint8_t x = 0; x < out_size; x++) {
        ...
        float xc_sin = yc_cos;
        for (uint8_t y = 0; y < out_size; y++) {
            uint8_t y_pos = static_cast<uint8_t>(static_cast<int>(xc_cos));
            uint8_t x_pos = static_cast<uint8_t>(static_cast<int>(xc_sin));
            if (
    ...
    for (uint8_t x = 0; x < out_size; x++) {
        ...
        for (uint8_t y = 0; y < out_size; y++) {
            ...
            if (
                ...
                if (
                    static_cast<int32_t>(x + x0) >= lib.ClipXMin &&
                    static_cast<int32_t>(x + x0) < lib.ClipXMax &&
                    static_cast<int32_t>(y + y0) >= lib.ClipYMin &&
                    static_cast<int32_t>(y + y0) < lib.ClipYMax
                ) {
                    uint8_t color = sprite->data[y_pos + x_pos * in_size];
                    *plot = color;
                }
...
void GraphY::gfz_RLETSprite_NoClip(const gfz_rletsprite_t *sprite, const uint32_t x, const uint8_t y) {
    gfz_Wait();
    const uint8_t* src_buf = sprite->data;
    uint8_t* dst_buf = get_vBuffer() + (y + (GFZ_LCD_HEIGHT * x));
    const uint32_t dst_jump = GFZ_LCD_HEIGHT - sprite->height;
...
void GraphY::gfz_RLETSprite(const gfz_rletsprite_t *__restrict sprite, const int32_t x, const int32_t y) {
    ...
    if (
        ...
        gfz_RLETSprite_NoClip(sprite, static_cast<uint32_t>(x), static_cast<uint8_t>(y));
        return;
    }
    gfz_Wait();
    ...
    const uint32_t min_clipX = (x < lib.ClipXMin) ? static_cast<uint32_t>(lib.ClipXMin - x) : 0;
    const uint32_t max_clipX = ((x + sprite->width) > lib.ClipXMax) ? static_cast<uint32_t>((x + sprite->width) - lib.ClipXMax) : 0;
    ...
    uint8_t *__restrict dst_buf = get_vBuffer() + (y0 + (GFZ_LCD_HEIGHT * (x0 + min_clipX)));
    const uint32_t dst_jump = GFZ_LCD_HEIGHT - sprite->height;
    ...
    const uint8_t maxY = static_cast<uint8_t>(lib.ClipYMax);
    const uint8_t minY = static_cast<uint8_t>(lib.ClipYMin);
    ...
    for (uint32_t posX = 0; posX < sizeX; posX++) {
        ...
        while (posY < sprite_height) {
            ...
            if (y0 + posY >= minY && y0 + posY + len <= maxY) {
