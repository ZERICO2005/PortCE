...
void GraphX::gfz_CopyRectangle(
    ...
    if (src_buf < dst_buf) {
        // copy backwards
        src_buf += GFZ_LCD_WIDTH * (height - 1);
        dst_buf += GFZ_LCD_WIDTH * (height - 1);
        for (uint32_t y_cord = 0; y_cord < height; y_cord++) {
...
void GraphX::gfz_ShiftDown(uint8_t pixels) {
    ...
    size_t copy_size = static_cast<size_t>(x1 - x0);
    size_t line_count = static_cast<size_t>(y1 - y0);

    uint8_t * buf = get_vBuffer() + x0 + ((y1 - 1) * GFZ_LCD_WIDTH);
...
void GraphX::gfz_ShiftUp(uint8_t pixels) {
    ...
    uint8_t * buf = get_vBuffer() + x0 + (y0 * GFZ_LCD_WIDTH);
    uint8_t const * src = buf + (shift * GFZ_LCD_WIDTH);
    uint8_t       * dst = buf;
    for (size_t i = 0; i < line_count; i++) {
        memcpy(dst, src, copy_size);
...
void GraphX::gfz_SetPixel_NoClip(uint32_t x, uint8_t y, uint8_t color) {
    if (x < GFZ_LCD_WIDTH && y < GFZ_LCD_HEIGHT) {
        get_vBuffer()[static_cast<uint32_t>(x) + (y * GFZ_LCD_WIDTH)] = color;
    }
}
...
void GraphX::gfz_SetPixel(uint32_t x, uint8_t y) {
    if (x < GFZ_LCD_WIDTH && y < GFZ_LCD_HEIGHT) {
        get_vBuffer()[static_cast<uint32_t>(x) + (y * GFZ_LCD_WIDTH)] = lib.Color;
    }
}

template<>
uint8_t GraphX::gfz_GetPixel(uint32_t x, uint8_t y) {
    return get_vBuffer()[(x & 0xFFFF) + (static_cast<uint32_t>(y) * GFZ_LCD_WIDTH)];
}
...
void GraphX::gfz_HorizLine_NoClip(uint32_t x, uint8_t y, uint32_t length) {
    uint8_t *fill = get_vBuffer() + (x + (static_cast<uint32_t>(y) * GFZ_LCD_WIDTH));
...
static void gfz_internal_PrintChar_NoClip(const char c, const uint8_t charWidth) {
    ...
    uint8_t *fillLinePtr = get_vBuffer() + (lib.TextXPos + (lib.TextYPos * GFZ_LCD_WIDTH));
...
gfz_sprite_t *GraphX::gfz_GetSprite(gfz_sprite_t *sprite_buffer, int32_t x, int32_t y) {
    ...
    const uint8_t* src_buf = get_vBuffer() + x + (y * GFZ_LCD_WIDTH);
...
void GraphX::gfz_Sprite_NoClip(const gfz_sprite_t *sprite, uint32_t x, uint8_t y) {
    ...
    uint8_t* dst_buf = get_vBuffer() + x + (y * GFZ_LCD_WIDTH);
...
void GraphX::gfz_Sprite(const gfz_sprite_t *sprite, int32_t x, int32_t y) {
    ...
    uint8_t* dst_buf = get_vBuffer() + (x + min_clipX) + ((y + min_clipY) * GFZ_LCD_WIDTH);
...
void GraphX::gfz_ScaledSprite_NoClip(
    ...
    const uint8_t* src_buf = sprite->data;
    uint8_t* buf_start = get_vBuffer() + x + (y * GFZ_LCD_WIDTH);
...
uint8_t GraphX::gfz_RotatedScaledSprite_NoClip(const gfz_sprite_t *sprite, uint32_t x0, uint8_t y0, uint8_t angle, uint8_t scale) {
    ...
    for (uint8_t x = 0; x < out_size; x++) {
        ...
        float xc_sin = yc_cos;
        for (uint8_t y = 0; y < out_size; y++) {
            uint8_t x_pos = static_cast<uint8_t>(static_cast<int>(xc_cos));
            uint8_t y_pos = static_cast<uint8_t>(static_cast<int>(xc_sin));
            if (
    ...
    for (uint8_t x = 0; x < out_size; x++) {
        ...
        for (uint8_t y = 0; y < out_size; y++) {
            ...
            if (
                ...
                if (
                    static_cast<int32_t>(y + x0) >= lib.ClipXMin &&
                    static_cast<int32_t>(y + x0) < lib.ClipXMax &&
                    static_cast<int32_t>(x + y0) >= lib.ClipYMin &&
                    static_cast<int32_t>(x + y0) < lib.ClipYMax
                ) {
                    uint8_t color = sprite->data[y_pos * in_size + x_pos];
                    *plot = color;
                }
...
void GraphX::gfz_RLETSprite_NoClip(const gfz_rletsprite_t *sprite, const uint32_t x, const uint8_t y) {
    ...
    gfz_Wait();
    const uint8_t* src_buf = sprite->data;
    uint8_t* dst_buf = get_vBuffer() + (x + (GFZ_LCD_WIDTH * y));
    const uint32_t dst_jump = GFZ_LCD_WIDTH - sprite->width;
        ...
        while (posX < sprite->width) {
            ...
            for (uint8_t r = 0; r < len; r++) {
                if (
                    ...
                    static_cast<int32_t>(posX) + x < lib.ClipXMax &&
                    static_cast<int32_t>(posY) + y >= lib.ClipYMin &&
                    static_cast<int32_t>(posY) + y < lib.ClipYMax
                ) {
                    *dst_buf = *src_buf;
                }
                ++dst_buf;
